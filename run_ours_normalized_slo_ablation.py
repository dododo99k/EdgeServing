"""
run_ours_normalized_slo_ablation.py

SLO ablation study for algorithm_ours_normalized.
...
def run_one_experiment(
    ...
    print(f"\n=== Running ours_normalized @ λ={lam}, SLO={slo_ms}ms ===")
    ...
    diag_with_config = os.path.join(
        diag_dir,
        f"multi_model_diag_ours_normalized_slo_{slo_label}ms.pkl",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="SLO ablation study for ours_normalized"
    )
    parser.add_argument(
        "--lambdas",
        type=str,
    ...
    print(f"\n{'='*60}")
    print(f"SLO Ablation Study for ours_normalized")
    print(f"{'='*60}")
