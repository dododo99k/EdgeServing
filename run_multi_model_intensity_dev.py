...
Run multi-model serving experiments to compare:
  ...
  - SLO violation ratio (violated / (completed + violated))
...
def main():
    ...
    for scheduler in schedulers:
        print(f"\nScheduler: {scheduler}")
        print(f"{'-'*60}")
        print(f"{'Lambda':<10} {'P95(ms)':<10} {'Violate%':<10} {'AvgExit':<10} {'Completed':<12} ")
        print(f"{'-'*60}")
