"""
run_early_exit_lowest_interference_exit_ablation.py

Ablation study for algorithm_early_exit_lowest_interference with different exit point configurations.
...
def run_one_experiment(
    ...
    print(f"\n=== Running early_exit_lowest_interference @ λ={lam}, exits={exit_config} ===")
    ...
    diag_with_config = os.path.join(
        diag_dir,
        f"multi_model_diag_early_exit_lowest_interference_exits_{exit_label}.pkl",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Ablation study for early_exit_lowest_interference with different exit point configurations"
    )
    parser.add_argument(
        "--lambdas",
        type=str,
    ...
    print(f"\n{'='*60}")
    print(f"Exit Point Ablation Study for early_exit_lowest_interference")
    print(f"{'='*60}")
    ...
    for exit_idx, exit_config in enumerate(exit_configs, 1):
        ...
        for lam in lambda_list:
            try:
                ...
                print(
                    ...
                    f"violated={metrics['num_violated']}, "
        ...
        print(f"{'-'*60}")
        print(f"{'Lambda':<10} {'P95(ms)':<10} {'violate_ratio%':<10} {'AvgExit':<10} {'Completed':<12} {'Violated':<10}")
        print(f"{'-'*60}")
...
if __name__ == "__main__":
    main()
    subprocess.run(["python", "plot_exit_ablation_comparison.py"], check=True)
