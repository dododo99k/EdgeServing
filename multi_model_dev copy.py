...
def algorithm_symphony(
    ...
    Symphony-style deferred batching:
      ...
      - For each model, pick the largest batch size B whose latest start time
        (based on the earliest deadline among the first B tasks and a profiled
        inference time) is still >= now.
      - If no batch size is feasible, pick the B with the largest latest start
        time (least overdue).
      - Only schedule a model when its chosen latest start time <= now;
        otherwise, defer by returning None.
    ...
    for m in model_names:
        ...
        best_B = None
        best_latest_start = None
        feasible_B = None
        feasible_latest_start = None
        ...
        for B in range(1, max_batch + 1):
            ...
            if best_latest_start is None or latest_start > best_latest_start:
                best_latest_start = latest_start
                best_B = B

            if latest_start >= now:
                if feasible_B is None or B > feasible_B:
                    feasible_B = B
                    feasible_latest_start = latest_start

        if feasible_B is not None:
            chosen_B = feasible_B
            chosen_latest_start = feasible_latest_start
        elif best_B is not None:
            chosen_B = best_B
            chosen_latest_start = best_latest_start
        else:
            continue
