"""
run_early_exit_lowest_interference_quantile_study.py

Quantile study for algorithm_early_exit_lowest_interference.
...
def run_one_experiment(
    ...
    print(f"\n=== Running early_exit_lowest_interference @ λ152={lam152}, quantile={slo_quantile} ===")
    ...
    diag_with_config = os.path.join(
        diag_dir,
        f"multi_model_diag_early_exit_lowest_interference_quantile_{slo_quantile}.pkl",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Quantile study for early_exit_lowest_interference"
    )
    parser.add_argument(
        "--lambda-ratio",
        type=str,
    ...
    print(f"\n{'='*70}")
    print(f"Quantile Study for early_exit_lowest_interference")
    print(f"{'='*70}")
